...
def setblock(x,y,color2):
  ...
  SendInput(Keyboard(KEY_T))
  time.sleep(0.04)
  SendInput(Keyboard(KEY_T, KEYEVENTF_KEYUP))
  time.sleep(0.04)

  for event in keyboard_stream( "/setblock {} 1 {} minecraft:wool {}".format(x,y,color) ):
    SendInput(event)
    time.sleep(0.001)
...
def main():
  bitmap_width = 1000
  bitmap_height = 1000
  #Partly stolen from https://github.com/PlaceDevs/place-gifs/blob/master/raw-bitmap-png.py
  header_size = 3

  #Read everything out of the bitmap file
  file = open("board-bitmap", 'rb')
  buffer = file.read()

  for i, byte in enumerate(buffer[(header_size + 1):]):
    x1 = (i * 2) % bitmap_width
    ...
    setblock(x1,y1,color1)
    setblock(x2,y2,color2)
